...
import os
import argparse
from model_fsgan import Generator, Discriminator
...
from utils.triplet_loss import TripletLoss, MMD_TripletLoss, MarginStep
...
checkpoint_path = "work_dir/checkpoints_v3/" + gan_type + "/" + data_path.split('/')[-1] + "/" + datetime.now().strftime("%Y%m%d%H%M")
...
for epoch in range(1, n_epochs + 1):
    for i, imgs in enumerate(dataloader):
        ...
        # 3.MMD thriplet loss
        margin = MarginStep(margin_min=0.5, margin_max=5, step_num=4, epochs=10000).get_margin(epoch)
        MMD_Thriplet_loss = Tensor(1).fill_(0)
        anc = fake_imgs_out[-1]
        pos = real_imgs_out[-1]
        neg = real_imgs_aug_out[-1]
        # criterionThrip = TripletLoss(margin=5)
        # criterionThrip = MMD_TripletLoss(margin=0.2)
        criterionThrip = MMD_TripletLoss(margin=margin)
        MMD_Thriplet_loss = criterionThrip(anc, pos, neg)

        # g_loss = 1. + 2. + 3.
        lambda_gan_g = 0.05
        lambda_gan_fm = 1.0
        lambda_mmd_thrip = 1.0
        GAN_G_loss = GAN_G_loss * lambda_gan_g
        GAN_Feature_Match_loss = GAN_Feature_Match_loss * lambda_gan_fm
        MMD_Thriplet_loss = MMD_Thriplet_loss * lambda_mmd_thrip

        g_loss = GAN_G_loss + GAN_Feature_Match_loss + MMD_Thriplet_loss
        g_loss.backward()
        opt_G.step()
        ...
        save_loss(checkpoint_path, epoch, n_epochs, i, len(dataloader), GAN_G_loss.item(), GAN_Feature_Match_loss.item(), MMD_Thriplet_loss.item(), g_loss.item(), d_loss.item())

        # save samples to /work_dir/checkpoints
        save_samples(sample_path, sample_interval, epoch, i, len(dataloader), fake_imgs.data)

        print(
            "[Epoch %d/%d] [Batch %d/%d] [GAN G loss: %f] [GAN Feature Match loss: %f] [MMD Thriplet loss: %f] [G loss: %f] [D loss: %f]"
            % (epoch, n_epochs, i, len(dataloader), GAN_G_loss, GAN_Feature_Match_loss, MMD_Thriplet_loss, g_loss.item(), d_loss.item()), end='\n'
        )
