...
# from attention import MultiHeadSelfAttention as SelfAttn
# from attention import MultiHeadDeformableAttention as SelfAttn
...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_size=64, conv_dim=64):
        ...
        # layer5.append(SpectralNorm(nn.Conv2d(curr_dim, curr_dim * 2, 4, 2, 1)))  # -> (1, 1024, 2, 2)
        # layer5.append(nn.LeakyReLU(0.1))
        # curr_dim = curr_dim * 2  # layer5 output dim 1024
        # attn3_dim = curr_dim  # attn3 dim 1024

        last.append(nn.Conv2d(curr_dim, 1, 4))  # -> (1, 1, 1, 1)
        # last.append(nn.Sigmoid())  # 去掉sigmoid函数
    ...
    def forward(self, x):
        ...
        # out = self.l5(out)  # (B, 1024, 2, 2) -> LeakyReLU
        # out, p3 = self.attn3(out)  # attn3=1024
        out = self.last(out)  # (B, 1, 1, 1)
        # out = self.avg(out)  # (B, 512, 4, 4) -> (B, 512, 1, 1)
        out = out.reshape(out.shape[0], -1)  # (B, 512)
        out = self.dense(out)  # (B, 512) -> (B, 1024)
...
if __name__ == '__main__':
    # test Generator
    G = Generator().cuda()
    z = torch.normal(0, 1, size=(32, 128)).cuda()
    fake = G(z)
    print(z.shape, fake.shape)
    summary(G, z.shape, device="cuda")

    # test Discriminator
    D = Discriminator().cuda()
    pred = D(fake)
    print(fake.shape, pred.shape, pred)
    summary(D, fake.shape, device="cuda")
