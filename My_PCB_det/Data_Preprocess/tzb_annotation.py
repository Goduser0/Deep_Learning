...
classes_path = "My_Datasets/Detection/PCB_瑕疵初赛样例集_VOC/cls_classes.txt"
...
PCB_DataSet_path = "My_Datasets/Detection/PCB_瑕疵初赛样例集_VOC"
    ...
    if annotation_mode == 0 or annotation_mode == 2:
        ...
        for image_set in PCB_Data_Sets:
            ...
            for image_id in image_ids:
                img_path = '%s/ImageSets/%s.bmp'%(os.path.abspath(PCB_DataSet_path), image_id)
                ...
                for obj in objects:
                    ...
                    [cls_id, x_cen, y_cen, w, h] = map(float, obj.split(' '))
                    # print(cls_id, x_cen, y_cen, w, h)
                    
                    left = int((x_cen - 0.5*w)*width)
                    right = int((x_cen + 0.5*w)*width)
                    top = int((y_cen - 0.5*h)*height)
                    bottom = int((y_cen + 0.5*h)*height)
                    
                    list_file.write(" " + ",".join([str(a) for a in [left, top, right, bottom]]) + ',' + str(int(cls_id)))
                    # print([str(a) for a in [left, top, right, bottom]], str(int(cls_id)))
