...
def calculate_kid_given_paths(paths, batch_size, cuda, dims, model_type='inception'):
    ...
    for j, pth in enumerate(pths):
        ...
        kid_values = polynomial_mmd_averages(act_true, actj, n_subsets=10)
...
def polynomial_mmd_averages(codes_g, codes_r, n_subsets=50, subset_size=1000,
    ...
    print(codes_g.shape, codes_r.shape)
    ...
    with tqdm(range(n_subsets), desc='MMD', file=output) as bar:
        for i in bar:
            g = codes_g[choice(len(codes_g), m//n_subsets, replace=False)]
            r = codes_r[choice(len(codes_r), m//n_subsets, replace=False)]
            o = polynomial_mmd(g, r, var_at_m=m, ret_var=ret_var)
            if ret_var:
                mmds[i], vars[i] = o
            else:
                mmds[i] = o
...
def kid_score_by_folder(paths):
    results = calculate_kid_given_paths(paths, 50, True, 2048)
    return results
