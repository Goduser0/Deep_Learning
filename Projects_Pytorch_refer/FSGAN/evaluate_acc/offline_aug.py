...
dataset_name = "NEU-10-r64"
initial_num = 10  # pad NEU-50-r64/train : 50x6cls
padding_num = 150  # multiple, for building few-shot CNN model
category = ['Cr', 'In', 'Pa', 'PS', 'RS', 'Sc']  # 6 cls name
...
def get_src_img(str_):
    ...
    :return: ndarray
    """
    img_root = Path('../dataset/NEU/' + dataset_name + '/train')
    img_root = img_root / str_
    img_list = []
    for image in img_root.glob('*.jpg'):
...
def data_padding_cls(str_, type_, seq):  # str_ = category{6}
    ...
    path = '../dataset/NEU/' + dataset_name + '-pad/' + type_ + '/train_' + str(initial_num * padding_num) + '/' + str_  # save path
    os.makedirs(path, exist_ok=True)
    for i in range(img_aug_concat.shape[0]):  # shape[0] = initial_num * padding_num
        save_path = os.path.join(path, str(i) + '_pad.jpg')
