...
if __name__ == "__main__":
    ...
    parser.add_argument("--add_train_csv", type=str, required=False, default="My_TAOD/dataset/filter/ConGAN/PCB_200[10-shot]/30.csv") #
    # "My_TAOD/dataset/filter/ConGAN/PCB_200[10-shot]/1500_renn.csv"
    # parser.add_argument("--add_train_csv", type=str, required=False, default="My_TAOD/dataset/filter/My_TAOD/dataset/filter/CycleGAN/PCB_200[10-shot]<-DeepPCB_Crop[200-shot]/30.csv") #
    ...
    add_train_csv = config.add_train_csv
    config.gan_type = add_train_csv.split('/')[-3]
    config.dataset_class = add_train_csv.split('/')[-2].split('[')[0]
    config.dataset_ratio = add_train_csv.split('/')[-2].split(']')[0].split('[')[-1]
    config.dataset_shot = add_train_csv.split('/')[-1].split('.')[0]
    ...
    # save_dir = f"./My_TAOD/Train_Classification/results/filter/{config.gan_type}/{add_train_csv.split('/')[-2]}/{config.classification_net}/{config.dataset_shot}/"
    ...
    dataset_train_dir = config.add_train_csv
