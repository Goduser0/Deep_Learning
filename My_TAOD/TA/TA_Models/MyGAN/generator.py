...
from model import MyGAN_Generator
import sys
...
def MyGAN_SampleGenerator(G_path, sample_size=100):
    #  G_path = "My_TAOD/TA/TA_Results/MyGAN/DeepPCB_Crop[10-shot]<-PCB_200[200-shot] 0 2025-03-09_12-03-47/models/10000_net_g_tar.pth"    
    device = "cuda:1"
    G = MyGAN_Generator()
    ...
    os.makedirs(f"My_TAOD/TA/TA_Results/MyGAN/{G_time}/samples/{dirname}", exist_ok=False)
    ...
    for batch_size in split_list:
        ...
        for img in tqdm.tqdm(imgs):
            ...
            filename = f"{i}.jpg"
            img_path = f"My_TAOD/TA/TA_Results/MyGAN/{G_time}/samples/{dirname}/{filename}"
            img.save(img_path)
    ...
    img_save_csv = f"My_TAOD/TA/TA_Results/MyGAN/{G_time}/samples/{dirname}/generate_imgs.csv"
    img_save_df.to_csv(img_save_csv)
    print("Generate Done!!!")
    return img_save_csv
    
if __name__ == "__main__":
    root_path = 'My_TAOD/TA/TA_Results/MyGAN'
    ...
    # 生成1500张样本
    for G_path in G_path_list:
        fake_path = MyGAN_SampleGenerator(f"{G_path}/models/2000_net_g_tar.pth", 200)