...
from TA_logger import DCGAN_logger
from TA_utils import DCGAN_record_data
...
# Save Directories
parser.add_argument("--save_rootdir", type=str, default="My_TAOD/TA/TA_Results/DCGAN")
# data loader
...
for epoch in tqdm.trange(1, config.num_epochs + 1, desc=f"[Epoch:{config.num_epochs}]On training"):
    ...
    for batch_idx, (raw_img, category_label) in enumerate(train_loader):
        ...
        real_adv_loss = nn.BCELoss()(real_img_out.flatten(), torch.ones(num).to(device))
        fake_adv_loss = nn.BCELoss()(fake_img_out.flatten(), torch.zeros(num).to(device))
        ...
        fake_adv_loss = nn.BCELoss()(fake_img_out.flatten(), torch.ones(num).to(device))
