...
mnist = datasets.MNIST(root='/home/zhouquan/MyDoc/Deep_Learning/My_Datasets/study', download=True, transform=transform)
...
if __name__ == '__main__': # 仅在当前文件中运行时才执行以下代码
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in data_loader:
            ...
            kl_loss += loss[2].item()
        print(f'Epoch {epoch+1} loss: {epoch_loss / len(mnist):.4f} recon_loss:{recon_loss / len(mnist):.4f} kl_loss:{kl_loss / len(mnist):.4f}')              
        ...
        with torch.no_grad():
            ...
            plt.savefig('demo_vae.jpg')
            plt.close()

    # 保存模型
    torch.save(model.state_dict(), 'vae_minst.pth')