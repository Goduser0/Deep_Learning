...
parse.add_argument("--checkpoint", type=str, default="checkpoints_v4")
...
checkpoint_path = "work_dir/" + config.checkpoint + "/" + gan_type + "/" + data_path.split('/')[-1] + "/" + datetime.now().strftime("%Y%m%d%H%M")
...
for epoch in range(1, n_epochs + 1):
    for i, imgs in enumerate(dataloader):
        ...
        for i_map in range(num_feature_maps):
            ...
            GAN_Feature_Match_loss += (fm_loss) * lambda_fm_weight[i_map]
